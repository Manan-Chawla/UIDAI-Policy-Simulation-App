...
#     """FIXED: Proper string date generation - NO numpy strftime errors"""
#     np.random.seed(42)
#     n_records = 50000
    
#     date_strings = []
#     for i in range(n_records):
#         year = np.random.randint(2025, 2027)
#         month = np.random.randint(1, 13)
#         day = np.random.randint(1, 29)  # Safe days
#         date_strings.append(f"{day:02d}-{month:02d}-{year}")
    
#     states = ['Uttar Pradesh', 'Bihar', 'Maharashtra', 'Karnataka', 'Madhya Pradesh', 
#               'Gujarat', 'West Bengal', 'Tamil Nadu', 'Rajasthan']
    
#     data = pd.DataFrame({
#         'date': date_strings,  
#         'state': np.random.choice(states, n_records),
#         'district': np.random.choice([f'District_{i}' for i in range(100)], n_records),
#         'pincode': np.random.randint(100000, 999999, n_records),
#         'age_0_5': np.random.randint(5, 50, n_records),
#         'age_5_17': np.random.randint(20, 100, n_records),
#         'age_18_greater': np.random.randint(10, 80, n_records)
#     })
    
#     # Your original calculations
#     data['total_population'] = data['age_0_5'] + data['age_5_17'] + data['age_18_greater']
#     data['child_ratio'] = (data['age_0_5'] + data['age_5_17']) / data['total_population'] * 100
#     data['adult_ratio'] = data['age_18_greater'] / data['total_population'] * 100
#     data['total_enrollments'] = np.random.randint(50, 500, n_records)
    
#     return data

# def calculate_priority_score(row, policy_params):
#     """Policy simulation scoring"""
#     base_score = (100 - row['child_ratio']) * row['total_enrollments'] / 100
#     school_boost = policy_params['school_multiplier'] if row['total_enrollments'] < 100 else 1
...
# st.markdown("**Hackathon-Ready Interactive Policy Analysis Tool**")

# # Sidebar - SIMPLIFIED (no date filtering to avoid datetime issues)
# st.sidebar.header("🔍 Data Filters")
# selected_state = st.sidebar.selectbox("State", ['All'] + sorted(data['state'].unique().tolist()))

# st.sidebar.header("Policy Simulator")
# policy_params = {
#     'school_multiplier': st.sidebar.slider("School Addition Multiplier", 1.0, 3.0, 1.2, 0.1),
#     'migration_rate': st.sidebar.slider("Migration Growth Rate (%)", 0.0, 50.0, 5.0, 1.0),
#     'enrollment_boost': st.sidebar.slider("Enrollment Boost (%)", 0.0, 100.0, 10.0, 5.0)
# }

# # Filter data - NO DATE FILTERING (simplest solution)
# filtered_data = data.copy()
# if selected_state != 'All':
#     filtered_data = filtered_data[filtered_data['state'] == selected_state].copy()
...
# total_enrollments = int(simulated_data['total_enrollments'].sum())
...
# with col1:
#     fig1 = px.histogram(simulated_data, x='priority_score', color='state', 
#                        title="Priority Score Distribution", nbins=30)
#     fig1.update_layout(height=400)
#     st.plotly_chart(fig1, use_container_width=True)

# with col2:
#     top_districts = simulated_data.groupby(['state', 'district']).agg({
#         'priority_score': 'mean',
#         'total_enrollments': 'sum'
#     }).round(2).reset_index().sort_values('priority_score', ascending=False).head(10)
#     st.dataframe(top_districts, use_container_width=True)

# # Policy Scenarios
# st.header("🔬 Policy Scenarios")
# tab1, tab2, tab3 = st.tabs(["Schools Impact", "Migration", "Live Simulator"])

# with tab1:
#     school_scenarios = {"No Change": 1.0, "+10 Schools": 1.5, "+20 Schools": 2.0, "+30 Schools": 2.5}
    
#     scenario_data = pd.DataFrame()
#     for name, mult in school_scenarios.items():
...
#     fig_school = px.box(scenario_data, x='scenario', y='priority_score', color='state')
...
#     fig_mig = px.line(
#         scenario_data.groupby(['scenario', 'state'])['priority_score'].mean().reset_index(),
#         x='scenario', y='priority_score', color='state'
#     )
...
#     col1, col2 = st.columns(2)
#     with col1:
#         live_school = st.slider("Live School Multiplier", 1.0, 3.0, 1.5)
#         live_mig = st.slider("Live Migration Rate (%)", 0.0, 50.0, 10.0)
        
#         live_params = {
#             'school_multiplier': live_school,
#             'migration_rate': live_mig,
#             'enrollment_boost': policy_params['enrollment_boost']
#         }
#         live_data = policy_simulation(filtered_data, live_params)
        
#         st.metric("Avg Priority Score", f"{live_data['priority_score'].mean():.0f}")
#         st.metric("High Priority Areas", len(live_data[live_data['priority_score'] > live_data['priority_score'].quantile(0.9)]))
    
#     with col2:
#         fig_live = px.bar(
#             live_data.groupby('state')['priority_score'].mean().reset_index(),
#             x='state', y='priority_score', title="Live State Priorities"
#         )
...
#     st.markdown(f"""
#     • **{row['state']} - {row['district']}**<br>
#       Score: {row['priority_score']:.0f} | Enrollments: {row['total_enrollments']}<br>
#       <b>Action:</b> Add {policy_params['school_multiplier']:.1f}x schools
#     """, unsafe_allow_html=True)
...
#     st.dataframe(simulated_data.head(1000), use_container_width=True)
